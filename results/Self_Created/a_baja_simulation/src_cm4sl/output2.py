"""
CarMaker Radar RSI Decoder
=======================================
Features: 
- Infinite auto-reconnect to CarMaker.
- Background UDP thread for continuous MATLAB publishing (50Hz).
- Exact zero-data string formatting for "no object" cycles.
"""
...
# ── Connection Settings ──────────────────────────────────────────────────────
HOST = "127.0.0.1"
PORT = 2210               # CarMaker TCP Port (Matches RSI GUI)
MATLAB_IP = "127.0.0.1"
MATLAB_PORT = 5005 # Simulink UDP Port
...
def main():
    ...
    print(f"UDP Publisher active. Sending data to MATLAB on port {MATLAB_PORT}...")

    while True:
        with udp_payload_lock:
            udp_payload_data = [0.0, 0.0, 0.0] # Reset to 0 when disconnected
        ...
        try:
            ...
            print("Connected successfully. Waiting for CarMaker data stream...")
            
            # --- FIX: Handshake removed! ---
            # CarMaker "User defined" sockets just stream raw data. 
            # Sending text strings here causes WinError 10053/10054 crashes.
            # tcp_sock.sendall(b"Subscribe RadarRSI\r\n") 
            # -------------------------------
            ...
            while True:
                try:
                    ...
                    if not chunk: 
                        print("Connection closed by CarMaker. Is the simulation paused or stopped?")
                        break
                    buffer += chunk
